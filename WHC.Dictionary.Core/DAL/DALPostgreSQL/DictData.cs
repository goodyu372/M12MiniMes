...
namespace WHC.Dictionary.DALPostgreSQL
{
	...
	/// DictData 的摘要说明。
	/// </summary>
    public class DictData : BaseDALPostgreSQL<DictDataInfo>, IDictData
	{
		#region 对象实例及构造函数
        ...
        {
            string sql = string.Format("select d.* from tb_DictData d inner join tb_DictType t on d.DictType_ID = t.ID where t.Code ='{0}'",
                dictCode);
