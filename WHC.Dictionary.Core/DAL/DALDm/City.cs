...
namespace WHC.Dictionary.DALDm
{
	...
	/// City 的摘要说明。
	/// </summary>
    public class City : BaseDALDm<CityInfo>, ICity
	{
		#region 对象实例及构造函数
        ...
        {
            string sql = string.Format("Select c.* from {1}.TB_City as c inner join {1}.TB_Province as p on c.ProvinceId=p.ID where ProvinceName='{0}' ", provinceName, this.Schema);
