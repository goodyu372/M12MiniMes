...
namespace WHC.Dictionary.DALDm
{
	...
	/// District 的摘要说明。
	/// </summary>
    public class District : BaseDALDm<DistrictInfo>, IDistrict
	{
		#region 对象实例及构造函数
        ...
        {
            string sql = string.Format("Select c.* from {1}.TB_District as c inner join {1}.TB_City as p on c.CityID=p.ID where CityName='{0}' ", 
                cityName, this.Schema);
