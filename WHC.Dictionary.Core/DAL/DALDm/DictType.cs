...
namespace WHC.Dictionary.DALDm
{
	...
	/// DictType 的摘要说明。
	/// </summary>
    public class DictType : BaseDALDm<DictTypeInfo>, IDictType
	{
		#region 对象实例及构造函数
        ...
        {
            string sql = string.Format("select Name,ID from {2} order by {0} {1}",
                SortField, IsDescending ? "DESC" : "ASC", this.tableName);
        ...
        {
            string sql = string.Format("select Name,ID from {3} where PID ='{2}' order by {0} {1}",
                SortField, IsDescending ? "DESC" : "ASC", PID, this.tableName);
        ...
        public List<DictTypeNodeInfo> GetTree()
        {
            ...
            string sql = string.Format("Select * From {0} Order By PID, Seq ", this.tableName);
            Database db = CreateDatabase();
        ...
        {
            string sql = string.Format("Select * From {0} Where PID='-1' Order By Seq  ", this.tableName);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(sql);
