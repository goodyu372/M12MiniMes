...
namespace WHC.Dictionary.DALDm
{
	...
	/// DictData 的摘要说明。
	/// </summary>
    public class DictData : BaseDALDm<DictDataInfo>, IDictData
	{
		#region 对象实例及构造函数
        ...
        {
            string sql = string.Format("select d.* from {1}.TB_DICTDATA d inner join {1}.tb_DictType t on d.DictType_ID = t.ID where t.Name ='{0}'", dictTypeName,this.Schema);
        ...
        {
            string sql = string.Format("select d.* from {1}.TB_DICTDATA d inner join {1}.tb_DictType t on d.DictType_ID = t.ID where t.Code ='{0}'", dictCode, this.Schema);
        ...
        {
            string sql = string.Format("select d.Name,d.Value from {2}.tb_DictData d inner join {2}.tb_DictType t on d.DictType_ID = t.ID order by d.{0} {1}", 
                SortField, IsDescending ? "DESC" :"ASC", this.Schema );
        ...
        {
            string sql = string.Format("select d.Name,d.Value from {3}.tb_DictData d inner join {3}.tb_DictType t on d.DictType_ID = t.ID where t.ID ='{0}' order by d.{1} {2}", 
                dictTypeId, SortField, IsDescending ? "DESC" : "ASC", this.Schema);
        ...
        {
            string sql = string.Format("select d.Name,d.Value from {3}.tb_DictData d inner join {3}.tb_DictType t on d.DictType_ID = t.ID where t.Name ='{0}' order by d.{1} {2}", 
                dictTypeName, SortField, IsDescending ? "DESC" : "ASC", this.Schema);
        ...
        {
            string sql = string.Format("select d.Name from {2}.tb_DictData d inner join {2}.tb_DictType t on d.DictType_ID = t.ID where t.Name ='{0}' and d.Value='{1}'",
                dictTypeName, dictValue, this.Schema);
