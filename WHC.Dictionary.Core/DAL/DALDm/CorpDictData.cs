...
namespace WHC.Dictionary.DALDm
{
    ...
    /// 公司字典数据
    /// </summary>
    public class CorpDictData : BaseDALDm<CorpDictDataInfo>, ICorpDictData
	{
		#region 对象实例及构造函数
        ...
        {
            string sql = string.Format(@"select d.* from {2}.TB_CorpDictData d inner join {2}.tb_DictType t on d.DictType_ID = t.ID 
            where t.Name ='{0}' AND d.Corp_ID='{1}' ", dictTypeName, corpId, this.Schema);
        ...
        {
            string sql = string.Format(@"select d.Name,d.Value from {4}.TB_CorpDictData d inner join {4}.tb_DictType t on d.DictType_ID = t.ID 
            where t.Name ='{0}' AND d.Corp_ID='{1}' order by d.{2} {3}",
                dictTypeName, corpId, SortField, IsDescending ? "DESC" : "ASC", this.Schema);
