...
namespace WHC.Dictionary.DALMySql
{
    /// <summary>
    /// 用户参数配置
    /// </summary>
    public class UserParameter : BaseDALMySql<UserParameterInfo>, IUserParameter
    {
        ...
        public bool SaveParamater(UserParameterInfo info)
        {
            ...
            if (!string.IsNullOrEmpty(info.Name))
            {
                ...
                //使用统一的数据库检查存储操作
                string sql = string.Format("Update {0} Set Content = {1}Content where Name={1}Name ", tableName, base.ParameterPrefix);
                DbCommand command = db.GetSqlStringCommand(sql);
                command.Parameters.Add(new MySqlParameter("Content", info.Content));
                command.Parameters.Add(new MySqlParameter("Name", settingsFilename));
        ...
        public string LoadParameter(string name, string creator = null)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                Database db = CreateDatabase();
                string sql = string.Format("Select Content from TB_UserParameter Where Name={0}Name ", base.ParameterPrefix);
                DbCommand command = db.GetSqlStringCommand(sql);
                command.Parameters.Add(new MySqlParameter("Name", settingsFilename));
