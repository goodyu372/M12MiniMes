...
namespace WHC.Attachment.DALDm
{
    /// <summary>
    /// 相关附件信息
    /// </summary>
	public class FileUpload : BaseDALDm<FileUploadInfo>, IFileUpload
	{
		#region 对象实例及构造函数
        ...
        public List<FileUploadInfo> GetAllByUser(string userId, string category, PagerInfo pagerInfo)
        {
            ...
            string condition = cond.BuildConditionSql(DatabaseType.Dm).Replace("Where", "");
        ...
        {
            string sql = string.Format("Select ID,FileName from {0} WHERE AttachmentGUID='{1}' ", this.tableName, attachmentGUID);
        ...
        {
            string sql = string.Format("Update {0} set DeleteFlag = 1 WHERE ID='{1}' ", this.tableName, id);
            Database db = CreateDatabase();
            DbCommand command = db.GetSqlStringCommand(sql);
