...
namespace WHC.Framework.ControlUtil
{
    ...
    /// PostgreSQL数据访问层的基类
    /// </summary>
    public abstract class BaseDALPostgreSQL<T> : AbstractBaseDAL<T>, IBaseDAL<T> where T : BaseEntity, new()
    {
        #region 构造函数
        ...
        public BaseDALPostgreSQL(string tableName, string primaryKey)
        ...
        public override int Insert2(Hashtable recordField, string targetTable, DbTransaction trans = null)
        {
            ...
            string sql = string.Format("INSERT INTO {0} ({1}) VALUES ({2});Select currval(pg_get_serial_sequence('{0}', '{3}'));", targetTable, fields, vals, primaryKey);
        ...
        public override List<T> FindWithPager(string condition, PagerInfo info, string fieldToSort, bool desc, DbTransaction trans = null)
        {
            ...
            string countSql = helper.GetPagingSql(true, DatabaseType.PostgreSQL);
            string strCount = SqlValueList(countSql, trans);
            info.RecordCount = Convert.ToInt32(strCount);

            string dataSql = helper.GetPagingSql(false, DatabaseType.PostgreSQL);
        ...
        public override DataTable FindToDataTable(string condition, PagerInfo info, string fieldToSort, bool desc, DbTransaction trans = null)
        {
            ...
            string dataSql = helper.GetPagingSql(false, DatabaseType.PostgreSQL);
            return GetDataTableBySql(dataSql.ToUpper(), trans);
        }
