...
namespace WHC.Framework.ControlUtil
{
    ...
    /// 数据访问层的基类
    /// </summary>
    public abstract class BaseDALDm<T> : AbstractBaseDAL<T>, IBaseDAL<T> where T : BaseEntity, new()
    {
        /// <summary>
        /// 数据库的模式名称
        /// </summary>
        ...
        public BaseDALDm(string tableName, string primaryKey, string schema = null)
            : base(tableName, primaryKey)
        {
            this.ParameterPrefix = ":";     //数据库参数化访问的占位符
            this.SafeFieldFormat = "{0}"; //防止和保留字、关键字同名的字段格式，如[value]

            #region 为达梦数据库设置统一的数据库模式Schema
            //由于达梦数据库的表模式Schema前缀必须带上，所以为表名增加模式前缀，如A.B这样格式表名，A代表模式前缀
            if (string.IsNullOrEmpty(schema))
            {
                string dbSchema = config.AppConfigGet("DM_Schema");
            ...
            {
                throw new ArgumentNullException("达梦数据库未设置统一的数据库模式Schema（配置参数DM_Schema），或者未在DAL类代码中指定schema。");
            }
            #endregion

            if (!base.tableName.Contains("."))
            {
                base.tableName = string.Format("{0}.{1}", this.Schema, base.tableName);
            }
        } 
        #endregion
        ...
        public override List<T> FindWithPager(string condition, PagerInfo info, string fieldToSort, bool desc, DbTransaction trans = null)
        {
            ...
            string countSql = helper.GetPagingSql(true, DatabaseType.Dm);
            string strCount = SqlValueList(countSql, trans);
            info.RecordCount = Convert.ToInt32(strCount);

            string dataSql = helper.GetPagingSql(false, DatabaseType.Dm);
